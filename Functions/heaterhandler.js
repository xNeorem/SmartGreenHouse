...
const log = "iot/logs"
const heater = "iot/devices/heater"
var currHeaterState = false
...
exports.handler = function(context, event) {
    ...
    if(temp <= 20){
        if(!currHeaterState){
            currHeaterState = true
            sendFeedbackMqtt(heater,"true")
        }
    }
    else if(temp >= 25){
        if(currHeaterState){
            currHeaterState = false
            sendFeedbackMqtt(heater,"false")
        }
    }

    console.log("TRIGGER "+_data);
    // sendFeedbackAmqp(log,"Temp : "+_data)
    // sendFeedbackMqtt(monitor,"Temp : "+_data)
};