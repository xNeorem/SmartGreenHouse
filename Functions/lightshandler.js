...
const lights = "iot/devices/lights"
var currLightsState = false
...
exports.handler = function(context, event) {
    ...
    if(temp <= 40){
        if(!currLightsState){
            currLightsState = true
            sendFeedbackMqtt(lights,"true")
        }
    }
    else if(temp >= 70){
        if(currLightsState){
            currLightsState = false
            sendFeedbackMqtt(lights,"false")
        }
    }

    console.log("TRIGGER "+_data);
    sendFeedbackAmqp(log,"Lightness : "+_data);
    sendFeedbackAmqp(monitor,"Lightness : "+_data)
};